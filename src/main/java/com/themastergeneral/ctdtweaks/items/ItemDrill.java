...
public class ItemDrill extends BasicItem 
{
                    ...
                    for(int zPos = aPos.getZ()-zRange; zPos <= aPos.getZ()+zRange; zPos++)
                    {
                        if(!(aPos.getX() == xPos && aPos.getY() == yPos && aPos.getZ() == zPos))
                        {
                            if(stack.getItemDamage() >= 1)
                            {
                                ...
                                {
                                    this.tryHarvestBlock(world, thePos, true, stack, player, 1);
                                }
                            }
                            else
                            {
    ...
    public boolean onBlockStartBreak(ItemStack stack, BlockPos pos, EntityPlayer player)
	{
        ...
        {
            //Block hit
            RayTraceResult ray = WorldHelper.getNearestBlockWithDefaultReachDistance(player.world, player);
            if(ray != null)
            {
                int side = ray.sideHit.ordinal();
                //Breaks the Blocks
                toReturn = this.breakBlocks(stack, 2, player.world, pos, side, player);
            }
        }
        return toReturn;
    }
